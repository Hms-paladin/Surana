    ...
    changeDynamic = (data, key,editTrue) => {
        ...
        if (filtererr.length > 0) {
            ...
            this.state.getInduction.map((data)=>{
                if(CLId===data.CLId && empId===data.empId){
                ...
                else if(CLId!==data.CLId || empId!==data.empId){
    ...
    parabox1 = (data,InduIdvaluearr) => {
        ...
        if (!this.state.colorchange.includes(data)) {
            ...
            var DeptId = this.state.responseDEPTID.find((val) => {
                ...
                var DeptId = this.state.responseDEPTID.find((data)=>{
                    if(self.state.addcandidatedata.department.value===data.DeptName){
                        return data.DeptId
                    }
                })

                console.log(DeptId.DeptId,"self.state.addcandidatedata.department.value")
    ...
    deptIdFun(empId){
        ...
        axios({
            ...
            response.data.data.map((data)=>{
                if(data.EmpId === empId){
                    self.state.addcandidatedata.department.value = data.DeptName
                    self.setState({})
                }
