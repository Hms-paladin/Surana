...
export default class DragdropTable extends Component {
  ...
  onDragEnd = (result) => {
    ...
    for (let i = result.source.index; i <= result.destination.index; i++) {
      dragid.push({ "projectTemplate_id": rows[i].id, "sort_order": i })
    }

    for (let j = result.destination.index; j <= result.source.index; j++) {
      dragid.push({ "projectTemplate_id": rows[j].id, "sort_order": j })
    }
                                ...
                                >
                                  ...
                                  <TableBody>
                                    <TableRow
                                      ...
                                      <TableCell key={index}>{row.stages}</TableCell>
                                      <TableCell key={index}>{row.substages}</TableCell>
                                      <TableCell key={index}>{row.TDays}</TableCell>
                                      <TableCell key={index}>{row.RBDays}</TableCell>
                                      <TableCell key={index}>{row.LinkToPrevious}</TableCell>
                                      <TableCell key={index}><VisibilityIcon className="tableeye_icon" onClick={(e)=>this.props.viewOpen(row.id)} /></TableCell>

                                      <TableCell className={"dragIconFelx"}>
                                        <EditIcon className="tableedit_icon" onClick={(e)=>this.props.editOpen(row.id,row)} />
                                        <DeleteIcon className="tabledelete_icon" onClick={(e)=>this.props.deleteOpen(row.id,row)} />
                                      </TableCell>
