...
// }
  ...
  viewopen =(data)=>{
      console.log(data,"dslfmdslfklsd")
    
    axios({
      method: 'post',
      url: apiurl+'/getdayreportcasedetails',
      headers: {
   ...
   console.log(response.data.data,"vbmcvbfkjghkfhit")
   this.setState({
     dayReportData:response.data.data[0],
     dayCounselData:response.data.data[0].counsel,
     dayOppositeData:response.data.data[0].opposite
   })
  ...
  checkdata=(id)=>{
    if(this.props.interimIdData.find(item=> item.DayreportId==id)){      
      this.props.interimIdData.filter(val =>  val.DayreportId ==id ? 
      this.setState({interimId:val.InterimId}) :"")
      
    }
    if(!this.state.viewRow.includes(id)){
        this.setState({
          viewRow:[id],
          // showrow:!this.state.show
        },()=>this.getInterim())        
      }else{
        this.setState({
          viewRow:[],
  ...
  axios({
    method: 'post',
    url: apiurl + "/viewInterim",
    data:{
      InterimId:this.state.interimId
    },
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .then(function (response) {
      console.log(response.data.data,"interimdatares")
      var interimdata = []
      response.data.data.map((data,index)=>{
        interimdata.push(data)               
      })
      self.setState({interimData:interimdata})
    })
    .catch(function (error) {
      console.log(error, "error");
    });
...
render(){
  console.log(this.state.dayReportData,"vbmcvbfkjghkfhit")
  ...
  var CollapseData=(
        ...
        <div className="Dayreport_collapsetable ml-2">
          <div className="Dayreport_row">
          <table>
            <tr>
              <th className="Dayreport_th">Interim Name</th>
              <th className="Dayreport_th">Application Date</th>
              <th className="Dayreport_th">Application No</th>
              <th className="Dayreport_th">Interim Details</th>
            </tr>
            {this.state.interimData.map((val)=>{
              return(
                  <tr>
                    <td style={{padding:"0px 0px 0px 25px"}}>{val.InterimName}</td>
                    <td>{val.ApplicationDate}</td>
                    <td>{val.ApplicationNo}</td>
                    <td>{val.Interimdetails}</td>
                  </tr>
              )
            })}
    ...
    <div className="heading_left" >
      ...
      headprops={this.props.tablehead}
        ...
        <div className="tableWrapper">
                     ...
                     <TableCell align="left" >
                      ...
                      <TableRow className="w-100 Dayreport_tablerow">
                          <TableCell colSpan={12}>
                            {this.state.viewRow.includes(row.id) ? CollapseData : ""}
                          </TableCell>
                      </TableRow>
                    </>
                  );
          ...
          dayReportData={this.state.dayReportData}
          dayCounselData={this.state.dayCounselData}
          dayOppositeData={this.state.dayOppositeData}
