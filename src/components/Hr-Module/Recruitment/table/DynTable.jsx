...
class RecruitmentTicketTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      order: 'asc',
  ...
  viewopen = (data) => {
    ...
    console.log("data in view open", data);

    axios({
      method: 'post',
      url: apiurl + '/generateTicketrequirement',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json'
      },
      data: {
        generateticketId: data.id
      }
    }).then((response) => {
      let RequirementData = []
      RequirementData = response.data.data[0]
...
export default withStyles(styles)(RecruitmentTicketTable);