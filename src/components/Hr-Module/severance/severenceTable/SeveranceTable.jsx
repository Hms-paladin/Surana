...
import '../../knowledge_mangement/knowledgeDynTable.css';
...
class SeveranceTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      order: 'asc',
  ...
  componentDidMount() {
    ...
    axios({
      ...
      .then(function (response) {
        ...
        var PendingData = response.data.data.map((data, index) => data.Status)
        self.setState({ Pending: PendingData })
      })
  ...
  render() {
    console.log(this.props.PendingDatas && this.props.PendingDatas, "PendingDatasasdfasdf")
    console.log(this.state.row, "rowdata")
    console.log(this.state.Pending, "pendingdata")
    var checkData = this.props.PendingDatas
                ...
                >
                  ...
                  <TableBody>
                    {stableSort(this.state.row, getSorting(this.state.order, this.state.orderBy))
                      ...
                      .map((row, index) => {
                        console.log(this.props.PendingDatas[index], "pendingStatusIndex")
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell align="left" >
                              <span
                                className={checkData && checkData[row.id] === 0 ? "text-danger hyper" : "text-success hyper"}
                                onClick={checkData && checkData[row.id] === 0 ? () => this.props.getData(row.id,row): null}>
                                {checkData && checkData[row.id] === 0 ? 'Pending' : 'Completed'}
                              </span>
                            </TableCell>
                            <TableCell align="left" style={{ paddingLeft: "10px" }}>{
                              <div className={"dis_flex-severance"}>
                                ...
                                {this.props.deleteclose === "deleteicon" ? null :
                                  <Tooltip title="Delete" placement="right" enterDelay={300}>
                                    <DeleteIcon className="sethover_background"
                                      onClick={(e) => this.opendeletemodel(row)}
                                    />
                                  </Tooltip>
                                }
                              </div>
...
export default withStyles(styles)(SeveranceTable);