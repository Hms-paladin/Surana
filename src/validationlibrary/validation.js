...
class ValidationLibrary extends Component {
  checkValidation(textValue, validatorsArray) {
    ...
    for (var valid in validatorsArray) {//check validations through array
      ...
      }   else if (validatorsArray[valid].name == 'characlimit') {
        
         if (textValue.length > parseInt(validatorsArray[valid].params)) {
          console.log("wdqwewqewqe",validatorsArray[valid].params)
          return { msg: "Length should not exceed " + validatorsArray[valid].params + " Numeric", state: false };
        }
      }
      ...
      //syed creates experience validattion
      ...
      else if (validatorsArray[valid].name == 'numSpecial') {
        var re = /^(?=.*?[1-9])[0-9()-]+[^<>{}\"/|;:.,~!?@#$%^=&*\\]\\\\()\\[¿§«»ω⊙¤°℃℉€¥£¢¡®©0-9_+]+$/;
        if (re.test(textValue) == false) {
          return { msg: "Please Enter Numeric Value only", state: false };
        }
      }
      ...
      }else if (validatorsArray[valid].name == 'creat') {
        var re = /^[a-zA-Z0-9!@#$&()*%-_~\\-`.+,/\"]*$/;
        if (re.test(textValue) == false) {
          return { msg: "Please Enter Alphabets only", state: false };
        }
